...
def create_sales_order():
	...
	try:
		...
		if not input_data.get('date'):
			return response('`date` is reuqired to create Sales Order', {}, False, 400)

		if not input_data.get('client_id'):
			return response('`client_id` is reuqired to create Sales Order', {}, False, 400)
		else:
			if not frappe.db.exists('Customer', input_data.get('client_id')):
				return response('Client : `{0}` does not exists'.format(input_data.get('client_id')), {}, False, 404)

		if not input_data.get('taxable_value'):
			return response('`taxable_value` is reuqired to create Sales Order', {}, False, 400)
		...
		so_doc.transaction_date = getdate(input_data.get('date'))
		so_doc.delivery_date = getdate(input_data.get('date'))
		so_doc.customer = input_data.get('client_id')
		so_doc.region = input_data.get('region')
		so_doc.executive = input_data.get('executive_id') or ''
		so_doc.reference_id = get_quotation_from_ro_id(input_data.get('ref_no')) or ''
		ro_item_row = so_doc.append('items')
		ro_item_row.qty = 1
		ro_item_row.item_code = albatross_service_item
		...
		if sales_taxes_and_charges_template:
			so_doc.taxes_and_charges = sales_taxes_and_charges_template
		else:
			so_doc.taxes_and_charges = ''
		if tax_category:
			so_doc.tax_category = tax_category
		else:
			so_doc.tax_category = ''
		...
		frappe.log_error("Character Length Exceeded", "Error in Sales Order creation: " + str(e)[:120])
		return response('Character Length Exceeded in Error Log', {}, False, 400)
