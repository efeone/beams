...
@frappe.whitelist()
def get_total_sales_order_amount(quotation_name):
    '''
    Method: Calculates the total amount of all Sales Orders linked to the Quotation using rounded_total.
    '''
    total_amount = frappe.db.sql("""
        SELECT SUM(rounded_total) FROM `tabSales Order`
