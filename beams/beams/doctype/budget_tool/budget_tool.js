		...
		fields: [
			{
				...
				reqd: 1,
				onchange: function () {
					let cost_head = d.get_value('cost_head');
					if (cost_head) {
						frappe.call({
							method: 'beams.beams.doctype.budget_tool.budget_tool.get_cost_head_details',
							args: {
								'cost_head': cost_head
							},
							callback: (r) => {
								if (r.message) {
									d.set_value('budget_group', r.message.budget_group);
									d.set_value('cost_category', r.message.cost_category);
								}
							}
						});
					}
...
function add_row_primary_action(frm, values) {
	if (frm.doc.budget && values.cost_head) {
		frappe.call({
